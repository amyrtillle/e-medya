...
<template>
  <div v-if="type != 'select' && type != 'checkbox'" class="em-input">
    ...
    <!-- input -->
    <input :id="name" class="input" :aria-label="ariaLabel" :class="type" :type="type" :placeholder="placeholder" :name="label" :required="required">
  </div>

  <div v-else-if="type == 'checkbox'" class="em-checkbox">
    <span class="checkbox">
      <input :id="name" class="input" :class="type" :type="type" :placeholder="placeholder" :name="label" :required="required">
      <svg>
  ...
  <div v-else class="em-select">
    ...
    <div ref="dropDown" class="dropdown-wrapper">
      <div ref="dropdDownWrap" class="dropdown-selected-option" :class="isOpen" @click="toggleDropDown()">
        <EmTextContent tag="span" class="typo-primary variant-tertiary s" :text="mappedSelectedOption" :value="mappedSelectedOption" />
      ...
      <div v-if="isDropDownVisible" class="options-wrapper">
        <div v-for="option in options" :key="option.value" class="option" @click="toggleOptionSelect(option)">
          <EmButton :value="option.value" :label="option.text" href="" />
        </div>
      </div>
    </div>
